	...
	my $element_name = $object->field_to_element($field);
        ...
        if (Data::OpenSocial::Types->is_primitive_type($type) || Data::OpenSocial::Types->is_simple_type($type)) {
            ...
            if ($type eq 'OpenSocial.AppdataEntry.Collection') {
                %serialized = ( map { ( $_->key, $_->value ) } @{ $object->entry } );
                return \%serialized;
            }
        }
        else {
            $serialized{$element_name} = $class->serialize($class, $object->$field);
        }
    }
...
sub parse_object {
    ...
    if ($class_type->isa('Data::OpenSocial::Appdata') && !exists $object->{entry}) {
        $object->{entry} = [];
        for (keys %$object) {
            push(@{$object->{entry}}, +{ key => $_, value => $object->{$_} });
            delete $object->{$_};
        }
    }
